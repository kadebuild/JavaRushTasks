...
public class Solution {
    ...
    public static int calculateHorsesFinished(List<Horse> horses) throws InterruptedException {
        int countFinished = 0;
        while (countFinished != countHorses)
        {
            if (horses.get(countFinished).isFinished)
                countFinished++;
            else
            {
                System.out.println("Waiting for " + horses.get(countFinished).getName());
                horses.get(countFinished).join();
                countFinished++;
            }
        }
